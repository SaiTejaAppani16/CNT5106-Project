...
public class peerProcess implements MessageConstants
{
	...
	public static void main(String[] args) 
	{
		...
		try
		{
			...
			while(true)
			{
				...
				if (!markedComplete && ownBitField.isCompleted()) {
					showLog(peerID + " has completed file download. Marking as complete...");
					markSelfAsCompleted();
					markedComplete = true;  // ✅ Prevents future logs
				}
	...
	public static void markSelfAsCompleted() {
		try {
			File inputFile = new File("PeerInfo.cfg");
			File tempFile = new File("PeerInfo_temp.cfg");
			...
			while ((currentLine = reader.readLine()) != null) {
				...
				if (tokens[0].equals(peerID)) {
					tokens[3] = "1";  // Mark this peer as complete
			...
			showLog("Error marking peer as completed: " + e.getMessage());
		}
	}
