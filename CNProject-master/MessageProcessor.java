...
public class MessageProcessor implements Runnable, MessageConstants 
{
	...
	private void sendPeice(Socket socket, DataMessage d, String remotePeerID) {
		try {
			byte[] bytePieceIndex = d.getPayload();
			if (bytePieceIndex == null || bytePieceIndex.length < 4) {
				peerProcess.showLog(peerProcess.peerID + " ERROR: Invalid piece request payload from Peer " + remotePeerID);
				return;
			}
	
			int pieceIndex = ConversionUtil.byteArrayToInt(bytePieceIndex);
			if (pieceIndex < 0 || pieceIndex >= peerProcess.ownBitField.getSize()) {
				peerProcess.showLog(peerProcess.peerID + " ERROR: Invalid piece index: " + pieceIndex);
				return;
			}
			...
			try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
				int offset = pieceIndex * CommonProperties.pieceSize;
				long fileLength = raf.length();
				int remaining = (int) (fileLength - offset);
				
				if (remaining <= 0) {
					peerProcess.showLog(peerProcess.peerID + " ERROR: Cannot read piece " + pieceIndex + ". Offset (" + offset + ") exceeds file length (" + fileLength + ")");
					return;
				}
				
				int actualSize = Math.min(CommonProperties.pieceSize, remaining);


				byte[] byteRead = new byte[actualSize];
				int noBytesRead = raf.read(byteRead, 0, actualSize);
	
				if (noBytesRead <= 0) {
					peerProcess.showLog(peerProcess.peerID + " ERROR : File could not be read properly or zero bytes read!");
					return;
				}
	
				byte[] buffer = new byte[MessageConstants.PIECE_INDEX_LEN + noBytesRead];
			...
			peerProcess.showLog(peerProcess.peerID + " UNEXPECTED ERROR in sendPeice: " + e.getMessage());
		}
	}
